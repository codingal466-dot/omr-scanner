...
function exportResultsCsv(rows) {
  ...
  const dataRows = rows.map(({ result, answerKeyName, answerKey }) => {
    const maxScore = answerKey ? answerKey.questions.reduce((sum, q) => sum + q.pointValue, 0) : result.questionResults.reduce(
    ...
    return rowToCsv([
      ...
      answerKeyName,
      new Date(Number(result.timestamp) / 1e6).toLocaleDateString()
    ]);
  });
  downloadCsv(`omr-results-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv`, [
    header,
    ...dataRows
  ]);
}
function exportResultPdf(result, breakdown, answerKey) {
  const date = new Date(
    Number(result.timestamp) / 1e6
  ).toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric"
  });
  const grade = breakdown.percentage >= 90 ? "A" : breakdown.percentage >= 80 ? "B" : breakdown.percentage >= 70 ? "C" : breakdown.percentage >= 60 ? "D" : "F";
  const questionRows = result.questionResults.map((qr) => {
    var _a;
    const correct = ((_a = answerKey == null ? void 0 : answerKey.questions.find((q) => q.questionNumber === qr.questionNumber)) == null ? void 0 : _a.correctAnswer) ?? "?";
    const statusLabel = qr.status === "correct" ? "✓ Correct" : qr.status === "incorrect" ? "✗ Incorrect" : qr.status === "empty" ? "— Empty" : "⚠ Anomaly";
    const statusColor = qr.status === "correct" ? "#16a34a" : qr.status === "incorrect" ? "#dc2626" : qr.status === "empty" ? "#6b7280" : "#d97706";
    return `<tr>
...
<h2>${(answerKey == null ? void 0 : answerKey.name) ?? "Unknown Answer Key"}</h2>
<div class="info-grid">
...
<div class="footer">Generated by EduScan OMR &bull; ${(/* @__PURE__ */ new Date()).toLocaleString()}</div>
</body>
</html>`;
...
function computeBreakdown(questionResults, answerKey) {
  let correct = 0n;
  let incorrect = 0n;
  let empty = 0n;
  let anomaly = 0n;
  ...
  for (const qr of questionResults) {
    const key = answerKey == null ? void 0 : answerKey.questions.find(
      (q) => q.questionNumber === qr.questionNumber
    );
    const pointValue = (key == null ? void 0 : key.pointValue) ?? 1;
  ...
  const percentage = maxPossibleScore > 0 ? Math.min(100, Math.max(0, totalScore / maxPossibleScore * 100)) : 0;
  return {
    questionResults,
...
export {
  exportResultPdf as a,
  computeBreakdown as c,
  exportResultsCsv as e
};
